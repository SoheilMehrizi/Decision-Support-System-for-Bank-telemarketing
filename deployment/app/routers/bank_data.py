...
#     summary="Add new bank data (single or batch)",tags=["telemarketing_data"]
# )
# def add_bank_data(
...
router = APIRouter(prefix="/bank_data", tags=["telemarketing_data"])

@router.post("/", response_model=Union[BankDataSchema, List[BankDataSchema]], status_code=status.HTTP_201_CREATED)
def add_bank_data(
    bank_data: Union[BankDataCreate, List[BankDataCreate]],
    ...
    result = repo.create_bank_data(bank_data)
    if result is None:
        raise HTTPException(status_code=500, detail="Failed to create bank data record(s).")
    return result

@router.post("/slide-window/", status_code=status.HTTP_200_OK)
def trigger_slide_window(
    current_user = Depends(get_current_superuser),
    db: Session = Depends(get_db),
):
    ...
    moved_count = repo.slide_window()
    if moved_count is None:
        raise HTTPException(status_code=500, detail="Failed to perform slide window operation.")
    return {"moved_records": moved_count}
